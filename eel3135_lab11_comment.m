...
% the result is padded with zeroes if X is less than N
%

% Answer on your comments: How is the DFT (using the FFT algorithm) 
% calculated  when N is smaller than the length of X? 
%the result is truncated to length N

% Answer in your comments: Why would you not want to always make N as large
% as possible?
%because its more computationally expensive. And, if length N is greater
%than X, the result just gets padded with zeroes.
...
%based on the 1000 DFT
% 0Hz, 448Hz, 1760Hz, 7040Hz
...
% magnitude plot.
%the magnitude is greater for all frequencies of the signal as the DFT
%increases in length to 500. After that, there is no significant difference
%in the magnitudes.
